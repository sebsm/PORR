    ...
    def init_ffa(self):
        ...
        for i in range(self.n):
            for h in range(1,size):
                comm.isend(i, dest = h) ##### MPI COMM SEND
            for j in range(self.d):
                for g in range(1,size):
                    comm.isend(j,dest = g) ##### MPI COMM SEND
                if True:
                    comm.irecv(source = h) ##### MPI COMM RECEIVE
                    comm.irecv(source = g) ##### MPI COMM RECEIVE
...
if rank == 0:
    ...
    comm.irecv(source=h, tag=MPI.ANY_TAG)
    comm.irecv(source=g, tag=MPI.ANY_TAG)
