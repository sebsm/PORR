...
class FireflyAlgorithm():
    ...
    def __init__(self, d, n, nfe, alpha, betamin, gamma, LB, UB, function):
        cdef self.d = d  # rozmiar wymiarów
        cdef self.n = n  # rozmiar populacji
        cdef self.nfe = nfe  # liczba iteracji
        cdef self.alpha = alpha  # parametr alpha
        cdef self.betamin = betamin  # parametr beta
        cdef self.gamma = gamma  # parametr gamma

        # sortowanie świetlików w zależności od funkcji dopasowania
        cdef self.Index = [0] * self.n
        cdef self.Fireflies = [[0 for i in range(self.d)]
                          for j in range(self.n)]  # init świetlików
        cdef self.Fireflies_tmp = [[0 for i in range(self.d)] for j in range(
            self.n)]  # tymczasowa populacja
        cdef self.Fitness = [0.0] * self.n  # wartosci dopasowania
        cdef self.I = [0.0] * self.n  # intensywność światła
        cdef self.nbest = [0.0] * self.n  # najlepsze znalezione rozwiązanie
        cdef self.LB = LB  # dolne ograniczenie
        cdef self.UB = UB  # górne ograniczenie
        cdef self.fbest = None  # najlepszy
        cdef self.evaluations = 0
        cdef self.Fun = function
        

    def init_ffa(self):
        for i in prange(self.n, nogil = True,num_threads=4):
            for j in prange(self.d, nogil = True,num_threads=4):
    ...
    def sort_ffa(self):  # sortowanie bąbelkowe
        ...
        for i in prange(0, (self.n - 1), nogil = True,num_threads=4):
            j = i + 1
            for j in prange(j, self.n, nogil = True,num_threads=4):
    ...
    def FindLimits(self, k):
        for i in prange(self.d, nogil = True,num_threads=4):
    ...
    def move_ffa(self):
        for i in prange(self.n, nogil = True,num_threads=4):
            ...
            for j in prange(self.n, nogil = True,num_threads=4):
                r = 0.0
                for k in prange(self.d, nogil = True,num_threads=4):
